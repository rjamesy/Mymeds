...
function CalendarGrid({
  ...
  const getDayStatus = (day: number): "none" | "partial" | "full" | "missed" => {
    ...
    // Exclude PRN/as-needed doses from calendar status indicators
    const dayLogs = allLogs.filter((l) => l.scheduledDate === dateStr && l.scheduledTime !== "PRN");
          ...
          return (
            <div key={log.id} className="px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-400">{med.dosage} {log.scheduledTime === "PRN" ? "· As needed" : `at ${formatTime(log.scheduledTime)}`}</p>
              </div>
