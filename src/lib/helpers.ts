...
export function getDaysSupply(med: Medication): number {
  if (med.frequency === "as_needed") {
    // As-needed meds have unpredictable consumption; just show stock count
    return med.currentStock > 0 ? Math.ceil(med.currentStock / med.tabletsPerDose) : 0;
  }
...
export function generateTodaysDoses(): DoseLog[] {
  ...
  const scheduledDoses: DoseLog[] = [];
  const prnDoses: DoseLog[] = [];

  for (const med of meds) {
    // As-needed medications get a single PRN entry
    if (med.frequency === "as_needed") {
      const existing = existingLogs.find(
        (l) => l.medicationId === med.id && l.scheduledTime === "PRN"
      );

      if (existing) {
        prnDoses.push(existing);
      } else {
    ...
    for (const time of med.scheduledTimes) {
      ...
      if (existing) {
        ...
        storage.upsertDoseLog(newLog);
        scheduledDoses.push(newLog);
      }
    }
  }

  // Scheduled doses sorted by time first, then PRN doses at the end
  const sorted = scheduledDoses.sort((a, b) => a.scheduledTime.localeCompare(b.scheduledTime));
  return [...sorted, ...prnDoses];
}
...
// Get adherence stats for a date range (excludes PRN/as-needed doses)
export function getAdherenceStats(startDate: string, endDate: string) {
  const logs = storage.getDoseLogs().filter(
    (l) => l.scheduledDate >= startDate && l.scheduledDate <= endDate && l.scheduledTime !== "PRN"
  );
